...
%The spectral width $\Delta k$ (or angular spread $\Delta \theta \simeq \Delta k/k_0$)  follows 
%\begin{equation}\label{eq:nx}
%    \Delta k^2  =\frac{1}{ N_\mathrm{max} -N_\mathrm{min}+1 }\sum_{n=N_\mathrm{min}}^{N_\mathrm{max}} k_n^2  \, .
%\end{equation}

%Hence,
%\begin{align}
%I_d(\mathbf{k}_d) &=  K  \mathcal{H}^{(D)}(\mathbf{k}_d) \Re\left[e^{\Gamma_0 G(0,x)}-e^{\Gamma_0 G(0,x_0)}\right]\nonumber\\
%&- K  \mathcal{H}^{(D)}(\mathbf{k}_d)\frac{\mathbf{k}_d^2}{2k_0}\int_{x_0}^x dz\Re\left[ e^{\Gamma_0 G(0,z)+i\frac{\mathbf{k}_d^2}{2k_0} (x-z)}\right]
%\, , \label{eq:idkd}
%\end{align}
